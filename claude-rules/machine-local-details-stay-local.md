...
This is not a PR-body rule. It is not a session-ID rule. It is the same test
...
A plan that will be committed is leaving the machine. Treat it as a PR body.
...
## Content that still does not belong in a PR
...
| Plan-internal names: `PR A`, `PR-5.md`, `plans/…` | Gitignored; the reader cannot resolve them. |
...
the commit strands. Do not put the stacking plan in the PR body.

## Closed loopholes

- "The chat rule is full session IDs, so maybe the PR too." Full IDs in chat
  are so *you* can open the session in the tool. A PR is not chat. A reader
  of the PR cannot open `bc047371-…`.
